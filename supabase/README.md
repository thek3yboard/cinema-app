...
3. En **Authentication > URL Configuration**, configurar **Site URL** como:
   - `https://cinema-center.vercel.app`

   Agregar además estas URLs a **Redirect URLs**:
   - `http://localhost:3000/en-US/auth/callback`
   - `http://localhost:3000/es-AR/auth/callback`
   - `https://cinema-center.vercel.app/en-US/auth/callback`
   - `https://cinema-center.vercel.app/es-AR/auth/callback`

   Para probar deployments Preview de Vercel se puede agregar temporalmente el patrón correspondiente de Vercel. En producción conviene mantener las dos URLs exactas anteriores. Si el callback solicitado no está permitido, Supabase ignora `redirectTo` y utiliza **Site URL**; por eso ese campo no debe quedar apuntando a localhost.
